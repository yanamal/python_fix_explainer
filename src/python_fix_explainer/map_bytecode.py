...
def compare_op_lists(op_list1: FlatOpsList, op_list2: FlatOpsList, changed_node_id: str, quick=False):
...
def gen_op_to_node_mapping(code_tree: muast.MutableAst, debug_mapping=False, quick=False):
    ...
    # if quick:
    #     # TODO - identify some subset of "important" nodes (edited ones?) and only worry about those
    #     deletion_order = []
    ...
    for del_node in deletion_order:
        ...
        curr_to_next_op_map, changed_ops = compare_op_lists(curr_ops, next_ops, annotation, quick=quick)
